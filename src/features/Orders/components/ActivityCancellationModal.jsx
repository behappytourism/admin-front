...
export default function ActivityCancellationModal({
        ...
        try {
            ...
            if (!cancellationTb) {
                const response = await axios.patch(
                    section === "b2c"
                        ? `/attractions/orders/b2c/${orderId}/cancel-activity/${bookingId}`
                        : `/attractions/orders/b2b/${orderId}/cancel-activity/${bookingId}`,
                    {
                ...
                setOrderData((prev) => ({
                    ...
                    cancellations:
                        prev?.cancellations.length > 0
                            ...
                            : [response.data?.cancellation],
                    refunds:
                        prev?.refunds.length > 0
                            ? [response.data?.refund, ...prev?.refunds]
                            : [response.data?.refund],
                    activities: prev?.activities.map((act) => {
                        console.log(act._id, response.data.activityId, "act");

                        if (
                            act?._id.toString() ==
                            response.data.activityId.toString()
                        ) {
                ...
                setOrderData((prev) => ({
                    ...prev,
                    cancellations: prev?.cancellations.map((canc) => {
                        console.log(canc._id, cancellationId, "cancellation");

                        if (canc._id.toString() == cancellationId.toString()) {
